...
const CommentItem = (props) => {
    ...
    for (let i = 0; i < depth - 1; i++) {
        dotArray.push(<Dot key={`${i}${depth}`} style={{ marginRight: 12, marginTop: 13 }} />)
    }

    return (
        <div>
            <div className={styles.wrapper}>
                <div className={styles.dotWrapper}>{dotArray}</div>
            ...
            {isOpen && children && children.map((el) => <CommentItem key={el.id} comment={el} />)}
        </div>
    )
}
