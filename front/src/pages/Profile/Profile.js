                   ...
                   <h3 className={styles.device__header}>Мои девайсы</h3>
                    {userDevices.map((device) => (
                        <div className={styles.device} key={device.id}>
                            <span className={styles.device__text}>OS: {device.os.family} {device.os.version}</span>
                            <span className={styles.device__text}>Браузер: {device.browser.name} {device.browser.version}</span>
                        </div>
                    ))}
                    <span className={styles.device__add}><img src={plusIcon} alt="addIcon" />Добавить новое устройство</span>
                </div>
            </div>
        </div>
    )
