              ...
              {allTags.map((item) => (
                  <div
                      ...
                      style={{
                          ...
                          backgroundColor: currentTags.some((el) => el.id === item.id) ? item.color : '#FFF'
                      }}
                  >
                      <span style={{ color: currentTags.some((el) => el.id === item.id) ? '#FFF' : item.color }}>{item.name}</span>
                  </div>
              ))}
          </div>
          <Button
        ...
        <div ref={modalRef} className={styles.bug_card}>
            <div className={styles.bug_card__header}>
                <div className={styles.bug_card__header__tags_list}>
                    {addTagIsOpen && (<AddTags setOpen={setAddTagIsOpen} setTags={setCurrentTags} tags={currentTags} allTags={allTags}/>)}
                    {
                        currentTags.length > 0 &&
                            ...
                            <button className={styles.bug_card__header__tags_list__empty} onClick={handleAddTag}>
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.bug_list}>
                {bugs.map((item) => (<BugCard allTags={allTags} bug={item} key={item.id} />))}
            </div>
        </div>
    )
};
